...
export const getProjectsCostsService = async (id: number) => {
	...
	const costsReference = costsReferenceArr.reduce(
	...
	const calculatedCosts = costsCategories.map((el) => {
		return {
			...
			puertas_y_ventanas:
				costsReference[el.puertas_y_ventanas].puertas_y_ventanas,
...
export const updateProjectCostsService = async (body: any, id: number) => {
	...
	return {
		calculatedProjectCosts: {
			project_id: id,
			muros_y_columnas:
				costsReference[body.muros_y_columnas].muros_y_columnas,
			techos: costsReference[body.techos].techos,
			puertas_y_ventanas:
				costsReference[body.puertas_y_ventanas].puertas_y_ventanas,
